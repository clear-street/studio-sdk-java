...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        when (this) {
            is JsonString -> value
            else -> throw StudioSdkInvalidDataException("Value is not a string")
        }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw StudioSdkInvalidDataException("'${name}' is not set")
            is JsonNull -> throw StudioSdkInvalidDataException("'${name}' is null")
            else -> throw StudioSdkInvalidDataException("'${name}' is invalid, received ${this}")
        }
